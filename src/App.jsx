...
export default function App() {
  ...
  useEffect(() => { const close = (event) => { if (event.key === 'Escape') { setDrawerOpen(false); setProfileOpen(false) } }; addEventListener('keydown', close); return () => removeEventListener('keydown', close) }, [])
  const navigate = (target) => { setDrawerOpen(false); setProfileOpen(false); location.hash = target; window.scrollTo({ top: 0, behavior: 'smooth' }) }
  ...
  return <div className="terminal-shell">
    <Sidebar page={route.page} count={positions.length} reviews={portfolio.pendingReviews.length} navigate={navigate} open={drawerOpen} onClose={() => setDrawerOpen(false)} refresh={market.refresh} stale={market.isStale} />
    <div className="terminal-workspace">
      <header className="terminal-topbar"><div className="topbar-title"><button className="menu-button" aria-label="Open navigation" onClick={() => setDrawerOpen(true)}>☰</button><div><p className="eyebrow">DSE Portfolio Terminal</p><h1>{pageTitle}</h1></div></div><div className="topbar-actions"><span className="market-state"><i className={market.isStale ? 'stale' : ''} />{market.isStale ? 'Cached prices' : 'DSE prices live'}</span><button className="theme-toggle" onClick={theme.cyclePreference} title={`Appearance: ${theme.preference}`} aria-label={`Change appearance. Current setting: ${theme.preference}`}>{theme.preference === 'system' ? '◐' : theme.resolvedTheme === 'light' ? '☀' : '☾'}</button><button className="btn buy topbar-trade" onClick={() => setTrade({ type: 'buy' })}>＋ Buy</button><button className="btn sell topbar-trade" onClick={() => setTrade({ type: 'sell' })} disabled={!positions.length}>− Sell</button><div className="profile-wrap"><button className="avatar" aria-label="Open profile menu" aria-expanded={profileOpen} onClick={() => setProfileOpen(!profileOpen)}>{(auth.user.displayName || auth.user.email || 'U').slice(0, 2).toUpperCase()}</button>{profileOpen && <div className="profile-menu"><strong>{auth.user.displayName || 'Investor'}</strong><small>{auth.user.email}</small><p>Appearance</p><div className="theme-options">{['system', 'light', 'dark'].map((choice) => <button key={choice} className={theme.preference === choice ? 'active' : ''} onClick={() => theme.setPreference(choice)}>{choice}</button>)}</div>{portfolio.pendingReviews.length > 0 && <button onClick={() => navigate('migration')}>Needs Review ({portfolio.pendingReviews.length})</button>}<button onClick={auth.logOut}>Sign out</button></div>}</div></div></header>
    ...
    <TradeModal mode={trade} positions={positions} lots={portfolio.lots} symbols={Object.keys(market.quotes).sort()} quotes={market.quotes} onClose={() => setTrade(null)} onBuy={portfolio.recordBuy} onSell={portfolio.recordSell} onUpdate={portfolio.updateTransaction} onDone={(target) => { setTrade(null); navigate(target) }} setError={setActionError} />
  </div>
}

function Sidebar({ page, count, reviews, navigate, open, onClose, refresh, stale }) {
  ...
  return <>{open && <button className="drawer-backdrop" aria-label="Close navigation" onClick={onClose} />}<aside className={`terminal-sidebar${open ? ' open' : ''}`}><div className="brand"><span>DT</span><div><strong>DSE Terminal</strong><small>Private portfolio</small></div><button className="sidebar-close" onClick={onClose} aria-label="Close navigation">×</button></div><p className="nav-label">Workspace</p><nav>{links.map(([id, icon, label]) => <button key={id} className={active === id ? 'active' : ''} onClick={() => navigate(id)}><span>{icon}</span>{label}{id === 'portfolio' && <b>{count}</b>}{id === 'migration' && reviews > 0 && <b className="review-count">{reviews}</b>}</button>)}</nav><div className="sidebar-status"><span><i className={stale ? 'stale' : ''} />{stale ? 'Using cached quotes' : 'Market prices live'}</span><button onClick={refresh}>Refresh market prices</button></div></aside><nav className="mobile-nav">{links.slice(0, 4).map(([id, icon, label]) => <button key={id} className={active === id ? 'active' : ''} onClick={() => navigate(id)}><span>{icon}</span>{label}</button>)}</nav></>
}
...
function Dashboard({ user, summary, positions, transactions, lots, fiscal, years, onFiscal, navigate, refresh, setTrade }) {
  return <div className="page-stack"><PageHeader eyebrow="Portfolio overview" title={`Good ${new Date().getHours() < 12 ? 'morning' : new Date().getHours() < 18 ? 'afternoon' : 'evening'}, ${(user.displayName || 'Investor').split(' ')[0]}`} text="Current holdings and realized performance for the selected Bangladesh fiscal year." actions={<><select className="input" value={fiscal} onChange={(e) => onFiscal(e.target.value)}>{years.map((year) => <option key={year}>{year}</option>)}</select><button className="btn" onClick={refresh}>↻ Refresh prices</button></>} />
    <Metrics items={[["Total Shares Held", number(summary.totalShares), `Across ${number(summary.totalStocks)} stock types`, undefined, () => navigate('portfolio')], ['Stock Types', number(summary.totalStocks), 'Distinct active DSE symbols', undefined, () => navigate('portfolio')], ['Current Cost Basis', money(summary.cost), 'Capital remaining in open lots'], ['Current Market Value', money(summary.marketValue), 'Latest available DSE prices'], ['Unrealized P/L', money(summary.unrealized), `${percent(summary.unrealizedPct)} after estimated sell fee`, summary.unrealized], [`${fiscal} Realized P/L`, money(summary.fiscalRealized), 'Completed dated sales', summary.fiscalRealized], ['Invested This FY', money(summary.fiscalInvestment), 'Buy value including commission'], ['Lifetime Realized P/L', money(summary.lifetimeRealized), 'All recorded sales', summary.lifetimeRealized]]} />
    {positions.some((position) => position.quoteSource !== 'live') && <div className="data-banner">Some positions use cached or unavailable quotes. Portfolio totals use the latest available price.</div>}
    <div className="dashboard-grid"><section className="panel"><PanelHeader title="Open Positions" subtitle={`${positions.length} active stocks • specific-lot cost basis`} action={<button className="btn small" onClick={() => navigate('portfolio')}>View portfolio</button>} /><PositionTable positions={positions} navigate={navigate} setTrade={setTrade} compact /></section><div className="side-stack"><PortfolioValue summary={summary} /><section className="panel"><PanelHeader title="Recent Activity" subtitle="Latest five transactions" /><ActivityList transactions={[...transactions].sort(sortTransactions).slice(0, 5)} lots={lots} /></section></div></div>
  </div>
}
...
function Portfolio({ positions, navigate, setTrade }) {
  ...
  const totalCost = positions.reduce((sum, item) => sum + item.cost, 0), totalValue = positions.reduce((sum, item) => sum + (item.marketValue || 0), 0)
  return <div className="page-stack"><PageHeader eyebrow="Current holdings" title="Portfolio" text="Aggregated open positions. Sales use exact purchase-lot selection." actions={<button className="btn buy" onClick={() => setTrade({ type: 'buy' })}>＋ Buy Stock</button>} /><div className="portfolio-summary"><div><span>Total shares</span><strong>{number(positions.reduce((sum, item) => sum + item.quantity, 0))}</strong></div><div><span>Stock types</span><strong>{number(positions.length)}</strong></div><div><span>Open cost</span><strong>{money(totalCost)}</strong></div><div><span>Market value</span><strong>{money(totalValue)}</strong></div></div><section className="panel"><div className="filters"><input className="input" aria-label="Search stock symbol" placeholder="Search symbol" value={search} onChange={(e) => setSearch(e.target.value)} /><select className="input" aria-label="Filter profit and loss" value={pl} onChange={(e) => setPl(e.target.value)}><option value="all">Profit & loss</option><option value="profit">Profit only</option><option value="loss">Loss only</option></select><select className="input" aria-label="Sort positions" value={sort} onChange={(e) => setSort(e.target.value)}><option value="market">Sort: Market value</option><option value="return">Sort: Return %</option><option value="symbol">Sort: Symbol</option></select></div><PositionTable positions={shown} navigate={navigate} setTrade={setTrade} /></section></div>
}
...
function StockDetail({ symbol, positions, lots, transactions, quotes, fiscal, navigate, setTrade }) {
  ...
  return <div className="page-stack"><button className="back" onClick={() => navigate('portfolio')}>← Back to Portfolio</button><section className="panel stock-hero"><div><p className="eyebrow">Stock position</p><h2>{symbol}</h2><strong className="hero-price">{quote?.ltp == null ? 'Price unavailable' : money(quote.ltp)}</strong><p className={quote?.change >= 0 ? 'positive' : 'negative'}>{quote?.change == null ? 'No live change' : `${quote.change >= 0 ? '▲' : '▼'} ${money(quote.change)}`}</p></div><div className="header-actions"><button className="btn buy" onClick={() => setTrade({ type: 'buy', symbol })}>＋ Buy More</button>{position && <button className="btn sell" onClick={() => setTrade({ type: 'sell', symbol })}>− Sell Shares</button>}</div></section>{position && <Metrics items={[['Open Quantity', number(position.quantity), `${openLots.length} purchase lots`], ['Cost Basis', money(position.cost), 'Remaining acquisition cost'], ['Market Value', money(position.marketValue), 'Gross current value'], ['Unrealized P/L', money(position.unrealized), percent(position.returnPct), position.unrealized], [`${fiscal} Realized`, money(fy), 'Completed sales', fy]]} />}
    <section className="panel"><PanelHeader title="Open Purchase Lots" subtitle="Exact lots available for future sales" /><LotList lots={openLots} symbol={symbol} setTrade={setTrade} /></section>
    <section className="panel"><PanelHeader title="Transaction History" subtitle={`All buys and sells for ${symbol}`} /><TransactionTable transactions={txs} lots={lots} navigate={navigate} setTrade={setTrade} /></section></div>
}
...
function TradeModal({ mode, positions, lots, symbols, quotes, onClose, onBuy, onSell, onUpdate, onDone, setError }) {
  ...
  useEffect(() => {
    ...
    if (mode.transaction) {
      ...
      setForm({ symbol: tx.symbol, date: tx.date || today(), quantity: tx.quantity, price: tx.price });
      const allocs = {};
      if (tx.type === 'sell' && tx.allocations) {
        tx.allocations.forEach((alloc) => {
          allocs[alloc.lotId] = alloc.quantity;
        });
      }
      setAllocations(allocs);
      setStep(1);
    } else {
      const symbol = mode.symbol || (mode.type === 'sell' ? positions[0]?.symbol : symbols[0]) || '';
      const qty = mode.lotId ? Math.min(100, lots.find((lot) => lot.id === mode.lotId)?.remainingQty || 100) : 100;
      setForm({ symbol, date: today(), quantity: qty, price: quotes[symbol]?.ltp ?? '' });
      setAllocations(mode.lotId ? { [mode.lotId]: qty } : {});
      setStep(1);
    }
  }, [mode])
  if (!mode) return null
  const isEdit = Boolean(mode.transaction)
  const selling = mode.type === 'sell', availableLots = lots.filter((lot) => lot.symbol === form.symbol && (lot.remainingQty > 0 || (isEdit && mode.transaction.allocations?.some(a => a.lotId === lot.id)))), available = availableLots.reduce((s, lot) => {
    let qty = Number(lot.remainingQty);
    if (isEdit && mode.transaction.type === 'sell') {
      const prevAlloc = mode.transaction.allocations?.find(a => a.lotId === lot.id);
      if (prevAlloc) qty += Number(prevAlloc.quantity);
    }
    return s + qty;
  }, 0), tx = { type: mode.type, quantity: form.quantity, price: form.price }, selected = Object.values(allocations).reduce((s, qty) => s + Number(qty || 0), 0), lotsById = Object.fromEntries(lots.map((lot) => [lot.id, lot])), reviewTx = { ...tx, allocations: Object.entries(allocations).filter(([, qty]) => qty > 0).map(([lotId, quantity]) => ({ lotId, quantity: Number(quantity) })) }, valid = form.symbol && form.date && Number(form.quantity) > 0 && Number(form.price) > 0 && (!selling || Number(form.quantity) <= available)
  const submit = async () => {
    setSaving(true);
    setError('');
    try {
      if (isEdit) {
        await onUpdate(mode.transaction, {
          date: form.date,
          quantity: form.quantity,
          price: form.price,
          allocations: selling ? reviewTx.allocations : undefined
        });
        onDone('transactions');
      } else {
        if (selling) {
          await onSell({ ...form, allocations: reviewTx.allocations });
          onDone('transactions');
        } else {
          await onBuy(form);
          onDone(`stock/${form.symbol}`);
        }
      }
    } catch (error) {
      setError(error.message || 'The transaction could not be saved.');
    } finally {
      setSaving(false);
    }
  }
  return <Modal isOpen title={isEdit ? `Edit ${selling ? 'Sell' : 'Buy'} Transaction` : (selling ? 'Record Sell Transaction' : 'Record Buy Transaction')} description={selling ? 'Allocate the exact shares sold against their purchase lots.' : 'A new purchase lot will be created for this transaction.'} onClose={onClose} footer={<div className="modal-actions">{step > 1 && <button className="btn" onClick={() => setStep(step - 1)}>Back</button>}<button className="btn" onClick={onClose}>Cancel</button>{(!selling || step === 3) ? <button className="btn buy" disabled={!valid || saving || (selling && selected !== Number(form.quantity))} onClick={submit}>{saving ? 'Saving…' : isEdit ? 'Save Changes' : selling ? 'Record Sale' : 'Record Buy'}</button> : <button className="btn buy" disabled={!valid || (step === 2 && selected !== Number(form.quantity))} onClick={() => setStep(step + 1)}>{step === 1 ? 'Select Purchase Lots' : 'Review Sale'}</button>}</div>}>
    ...
    {step === 1 && <div className="form-grid"><label>Stock<select className="input" value={form.symbol} disabled={isEdit} onChange={(e) => { const symbol = e.target.value; setForm({ ...form, symbol, price: quotes[symbol]?.ltp ?? '' }); setAllocations({}) }}>{(selling ? positions.map((p) => p.symbol) : symbols).map((symbol) => <option key={symbol}>{symbol}</option>)}</select></label><label>Trade date<input className="input" type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></label><label>Quantity<input className="input" type="number" min="1" max={selling ? available : undefined} value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} /><small>{selling ? `${number(available)} shares available` : ''}</small></label><label>Price per share<input className="input" type="number" min="0.01" step="0.01" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} /></label><Calculation tx={tx} /></div>}
    {selling && step === 2 && <div><p className="allocation-note">Allocate {number(form.quantity)} shares. Selected: <b>{number(selected)}</b></p><div className="lot-list">{availableLots.map((lot) => {
      let maxQty = Number(lot.remainingQty);
      if (isEdit && mode.transaction.type === 'sell') {
        const prevAlloc = mode.transaction.allocations?.find(a => a.lotId === lot.id);
        if (prevAlloc) maxQty += Number(prevAlloc.quantity);
      }
      return <label key={lot.id}><span><b>{prettyDate(lot.purchaseDate)}</b><small>{money(lot.price)} • {number(maxQty)} available</small></span><input className="input" type="number" min="0" max={maxQty} value={allocations[lot.id] || 0} onChange={(e) => setAllocations({ ...allocations, [lot.id]: Math.min(Number(e.target.value), maxQty) })} /></label>
    })}</div></div>}
...
function PortfolioValue({ summary }) { const max = Math.max(summary.cost, summary.marketValue, 1); return <section className="panel value-panel"><PanelHeader title="Portfolio Value" subtitle="Market value compared with cost basis" /><div><span>Cost basis <b>{money(summary.cost)}</b></span><span>Market value <b>{money(summary.marketValue)}</b></span><div className="value-bar"><i style={{ width: `${summary.marketValue / max * 100}%` }} /></div><small>{percent(summary.unrealizedPct)} estimated return after sell fee</small></div></section> }
function ActivityList({ transactions, lots }) { const byId = Object.fromEntries(lots.map((lot) => [lot.id, lot])); if (!transactions.length) return <Empty title="No recent activity" text="Your latest trades will appear here." />; return <div className="activity-list">{transactions.map((tx) => { const pl = realizedProfit(tx, byId); return <article className="activity" key={tx.id}><i className={tx.type}>{tx.type === 'buy' ? 'B' : 'S'}</i><div><strong>{tx.type === 'buy' ? 'Bought' : 'Sold'} {number(tx.quantity)} {tx.symbol}</strong><small>{prettyDate(tx.date)} • {money(tx.price)} / share</small></div><span>{money(grossAmount(tx))}{pl != null && <small className={pl >= 0 ? 'positive' : 'negative'}>{money(pl)}</small>}</span></article> })}</div> }
function LotList({ lots, symbol, setTrade }) { if (!lots.length) return <Empty title="No open purchase lots" text="All recorded shares have been sold." />; return <><div className="table-wrap desktop-records"><table><thead><tr><th>Purchase date</th><th>Price</th><th>Original</th><th>Remaining</th><th>Source</th><th /></tr></thead><tbody>{lots.map((lot) => <tr key={lot.id}><td>{prettyDate(lot.purchaseDate)}</td><td>{money(lot.price)}</td><td>{number(lot.originalQty)}</td><td>{number(lot.remainingQty)}</td><td><Pill>{lot.source}</Pill></td><td><button className="btn small sell" onClick={() => setTrade({ type: 'sell', symbol, lotId: lot.id })}>Sell lot</button></td></tr>)}</tbody></table></div><div className="mobile-records">{lots.map((lot) => <article className="record-card" key={lot.id}><header><div><strong>{prettyDate(lot.purchaseDate)}</strong><Pill>{lot.source}</Pill></div><span><small>Remaining</small>{number(lot.remainingQty)}</span></header><div className="record-grid"><span><small>Price</small><b>{money(lot.price)}</b></span><span><small>Original quantity</small><b>{number(lot.originalQty)}</b></span></div><button className="btn sell" onClick={() => setTrade({ type: 'sell', symbol, lotId: lot.id })}>Sell from this lot</button></article>)}</div></> }
function PositionTable({ positions, navigate, setTrade }) { if (!positions.length) return <Empty title="No open positions" text="Record a buy transaction to create your first purchase lot." />; return <><div className="table-wrap desktop-records"><table><thead><tr><th>Symbol</th><th>LTP</th><th>Day</th><th>Quantity</th><th>Lots</th><th>Cost Basis</th><th>Market Value</th><th>Unrealized P/L</th><th>Return</th><th /></tr></thead><tbody>{positions.map((p) => <tr key={p.symbol}><td><button className="symbol-link" onClick={() => navigate(`stock/${p.symbol}`)}>{p.symbol}</button></td><td>{p.ltp == null ? '—' : money(p.ltp)}</td><td className={p.dayChange >= 0 ? 'positive' : 'negative'}>{p.dayChange == null ? '—' : money(p.dayChange)}</td><td>{number(p.quantity)}</td><td>{p.lots}</td><td>{money(p.cost)}</td><td>{p.marketValue == null ? '—' : money(p.marketValue)}</td><td className={p.unrealized >= 0 ? 'positive' : 'negative'}>{p.unrealized == null ? '—' : money(p.unrealized)}</td><td>{p.returnPct == null ? '—' : percent(p.returnPct)}</td><td><button className="btn small sell" onClick={() => setTrade({ type: 'sell', symbol: p.symbol })}>Sell</button></td></tr>)}</tbody></table></div><div className="mobile-records">{positions.map((p) => <article className="record-card" key={p.symbol}><header><div><button className="symbol-link" onClick={() => navigate(`stock/${p.symbol}`)}>{p.symbol}</button><Pill>{p.quoteSource}</Pill></div><span><small>Total quantity</small>{number(p.quantity)}</span></header><div className="record-grid"><span><small>LTP</small><b>{p.ltp == null ? '—' : money(p.ltp)}</b></span><span><small>Purchase lots</small><b>{p.lots}</b></span><span><small>Cost basis</small><b>{money(p.cost)}</b></span><span><small>Market value</small><b>{p.marketValue == null ? '—' : money(p.marketValue)}</b></span><span><small>Unrealized P/L</small><b className={p.unrealized >= 0 ? 'positive' : 'negative'}>{p.unrealized == null ? '—' : money(p.unrealized)}</b></span><span><small>Return</small><b className={p.returnPct >= 0 ? 'positive' : 'negative'}>{p.returnPct == null ? '—' : percent(p.returnPct)}</b></span></div><footer><button className="btn" onClick={() => navigate(`stock/${p.symbol}`)}>Details</button><button className="btn buy" onClick={() => setTrade({ type: 'buy', symbol: p.symbol })}>Buy more</button><button className="btn sell" onClick={() => setTrade({ type: 'sell', symbol: p.symbol })}>Sell</button></footer></article>)}</div></> }
function TransactionTable({ transactions, lots, navigate, onDelete, setTrade }) { const byId = Object.fromEntries(lots.map((lot) => [lot.id, lot])); if (!transactions.length) return <Empty title="No matching transactions" text="Record a buy or adjust the filters." />; return <><div className="table-wrap desktop-records"><table><thead><tr><th>Date</th><th>Type</th><th>Symbol</th><th>Quantity</th><th>Price</th><th>Gross</th><th>Commission</th><th>Net</th><th>Realized P/L</th><th>Source</th>{onDelete && <th>Actions</th>}</tr></thead><tbody>{transactions.map((tx) => { const pl = realizedProfit(tx, byId); return <tr key={tx.id}><td className={!tx.date ? 'warning' : ''}>{prettyDate(tx.date)}</td><td><Pill type={tx.type}>{tx.type}</Pill></td><td><button className="symbol-link" onClick={() => navigate(`stock/${tx.symbol}`)}>{tx.symbol}</button></td><td>{number(tx.quantity)}</td><td>{money(tx.price)}</td><td>{money(grossAmount(tx))}</td><td>{money(commission(tx))}</td><td>{money(netAmount(tx))}</td><td className={pl == null ? '' : pl >= 0 ? 'positive' : 'negative'}>{pl == null ? '—' : money(pl)}</td><td><Pill>{tx.source}</Pill></td>{onDelete && <td><div className="table-actions" style={{ display: 'flex', gap: '4px' }}><button className="btn small" onClick={() => setTrade({ type: tx.type, transaction: tx })}>Edit</button><button className="btn small delete" onClick={() => onDelete(tx)}>Delete</button></div></td>}</tr> })}</tbody></table></div><div className="mobile-records">{transactions.map((tx) => { const pl = realizedProfit(tx, byId); return <article className="record-card" key={tx.id}><header><div><button className="symbol-link" onClick={() => navigate(`stock/${tx.symbol}`)}>{tx.symbol}</button><span><Pill type={tx.type}>{tx.type}</Pill> <Pill>{tx.source}</Pill></span></div><span><small>{tx.type === 'buy' ? 'Bought' : 'Sold'}</small>{number(tx.quantity)}</span></header><div className="record-grid"><span><small>Trade date</small><b className={!tx.date ? 'warning' : ''}>{prettyDate(tx.date)}</b></span><span><small>Price / share</small><b>{money(tx.price)}</b></span><span><small>Gross</small><b>{money(grossAmount(tx))}</b></span><span><small>Commission</small><b>{money(commission(tx))}</b></span><span><small>Net amount</small><b>{money(netAmount(tx))}</b></span><span><small>Realized P/L</small><b className={pl == null ? '' : pl >= 0 ? 'positive' : 'negative'}>{pl == null ? '—' : money(pl)}</b></span></div><footer><button className="btn" onClick={() => navigate(`stock/${tx.symbol}`)}>View stock</button>{onDelete && <><button className="btn buy" onClick={() => setTrade({ type: tx.type, transaction: tx })}>Edit</button><button className="btn delete" onClick={() => onDelete(tx)}>Delete entry</button></>}</footer></article> })}</div></> }
